...
<Callout type='info'>
  <div className='text-sm'>
    ...
    specification](https://github.com/ga4gh/workflow-execution-service-schemas/blob/3a832abeeaffc1cb8d79045ba0d42b790773d9/openapi/workflow_execution_service.openapi.yaml#L234C1-L320C24).
  </div>
</Callout>
...
The `baseURL` property specifies the base URL for the TES API. The component will append `/runs` to this URL to form the complete endpoint for creating a task run.

## Usage Notes

1. **API Endpoint**: The component uses the `baseURL` + `/runs` as the endpoint for creating a new task run via a POST request.
