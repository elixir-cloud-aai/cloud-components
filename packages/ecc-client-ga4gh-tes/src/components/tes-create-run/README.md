...
The `executors` attribute is a list of objects that specify the configuration for executing a task. Each object in the list represents an executor with its own set of parameters. In the provided code snippet, there is one executor defined with the following configuration:

- `command`: Specifies the command to be executed. E.g `["/bin/md5", "/data/file1"]`.

- `env`: Specifies the environment variables for the executor.

- `image`: Specifies the Docker image to use for the executor. E.g `"ubuntu:20.04"`.

- `stderr`: Specifies the file path to redirect the standard error output. E.g `"/tmp/stderr.log"`.

- `stdin`: Specifies the file path to use as standard input for the executor. E.g `"/data/file1"`.

- `stdout`: Specifies the file path to redirect the standard output. E.g `"/tmp/stdout.log"`.

- `workdir`: Specifies the working directory for the executor. E.g `"/data/"`.

You can pass multiple executors to the component by adding additional objects to the `executors` list with their respective configurations.

#### inputs

The `inputs` attribute is an array of `InputData` objects that represent the input data for the task. Each `InputData` object has two properties:

- `url`: Represents the URL of the input data. It is of type `string`.

- `path`: Represents the path where the input data is stored. It is of type `string`.
...
"inputs": [
  {
    "url": "https://example.com/data/file1",
    "path": "/data/file1"
  },
  {
    "url": "https://example.com/data/file2",
    "path": "/data/file2"
  }
]
```
...
The `outputs` attribute is an array of `OutputData` objects that represent the output data generated by the task. Each `OutputData` object has three properties:

- `path`: Represents the path where the output data is stored. It is of type `string`.

- `url`: Represents the URL of the output data. It is of type `string`.

- `type`: Represents the type or format of the output data. It is of type `string`.
...
"outputs": [
  {
    "path": "/data/output1",
    "url": "https://example.com/data/output1",
    "type": "text/plain"
  },
  {
    "path": "/data/output2",
    "url": "https://example.com/data/output2",
    "type": "image/png"
  }
]
