...
describe("when array template is rendered", () => {
  ...
  it.skip("delete button should delete the correct instance", async () => {
    ...
    formComponent.clickButton(
      formComponent.arrayDeleteButton(false, arrayItems[1])
    );
    await formComponent.form.updateComplete;
  ...
  it("add button should work properly", async () => {
    ...
    formComponent.clickButton(
      formComponent.arrayAddButton(false, firstArrayField)
    );
    await formComponent.form.updateComplete;
...
describe.only("when group template is rendered", () => {
  ...
  it("should render children fields correctly", async () => {
    ...
    expect(formComponent.nonCollapsibleGroup(false, groupTemplate)).to.be.not
      .exist;
    expect(formComponent.inputField(true, collapsibleGroup)).to.have.lengthOf(
      3
    );
    expect(formComponent.switchField(true, collapsibleGroup)).to.have.lengthOf(
      1
    );
    expect(
      formComponent.inputFileField(true, collapsibleGroup)
    ).to.have.lengthOf(1);
  });
});
